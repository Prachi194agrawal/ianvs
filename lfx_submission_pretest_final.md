...
\textbf{Resume:} & \href{https://drive.google.com/file/d/1juIiaQFPR6ZVsdHrVlNag5lOYuVPk59G/view?usp=sharing}{View Resume (Google Drive)} \\
\textbf{GitHub:} & \href{https://github.com/Prachi194agrawal}{github.com/Prachi194agrawal} \\
...
**CNCF KubeEdge Contributor** (2025--Present, Linux Foundation)
...
- **RTT = 10ms**: Speculation saves ~150ms per 8-token batch $\rightarrow$ **2.5x speedup**
...
No existing Ianvs benchmark quantifies **when speculation helps vs. hurts** across different network conditions. This project fills that gap with a reproducible simulation framework and a concrete adaptive algorithm.
...
- **Total**: ~200 configurations, executable in 24--48 hours
...
**Statistical Rigor**: Each configuration run 3 times; report mean ± standard deviation. Use Welch's t-test for significance (p < 0.05).
...
## 5. Minimal Landing Plan (Week 1 Shippable)

> **Goal**: A working single-host RTT simulation benchmark merged into Ianvs within Week 1.

**Week 1 Deliverable --- First Mergeable PR**:

1. Simulate RTT using `tc`/`netem` on loopback interface
2. Run baseline cloud-only autoregressive decoding, measure TTFT
3. Run fixed-K speculative decoding (K=4, K=8), measure TTFT
4. Output: CSV comparing baseline vs. speculative at 5 RTT values

**Initial PR contents**:

- `benchmarkingjob_speculative.yaml` --- new benchmark config
- `network_emulator.py` --- RTT simulation wrapper using `tc`/`netem`
- `speculative_decoder.py` --- baseline vs. fixed-K comparison
- `testenv_speculative.yaml` --- test environment with RTT/BW parameters

**Acceptance Criteria**:

- Single command: `ianvs -f benchmarkingjob_speculative.yaml` produces comparison CSV
- RTT injection validated: measured RTT matches config within ±5ms
- Runs on CPU-only machine (no GPU required)

---

## 6. Full Implementation Milestones (12-Week Plan)
...
- Network emulator validated: injected RTT matches measured RTT within ±5ms
...
## 7. Innovative Acceleration Idea: Adaptive Hybrid Speculative Decoding (AHSD)

### Overview

Standard speculative decoding uses a **fixed** draft size K regardless of network conditions. This is suboptimal for cloud-edge deployments where RTT and bandwidth vary. AHSD introduces **two lightweight mechanisms** --- dynamic K adjustment and confidence-based early exit --- that adapt to changing constraints with minimal code complexity.
...
### Mechanism 2: Confidence-Based Early Exit

**What Changes**: Before sending all K draft tokens to the cloud, AHSD checks each draft token's top-1 logit confidence (softmax probability). Tokens below a confidence threshold $\tau$ are truncated --- the draft batch is cut short at the first low-confidence token, reducing network payload without any additional model.

**Implementation**: A simple argmax + softmax check on the draft model's existing logits. **Zero additional parameters, zero additional dependencies.**

```python
def filter_drafts(logits, threshold=0.7):
    """Truncate draft at first low-confidence token."""
    probs = torch.softmax(logits, dim=-1)
    confidences = probs.max(dim=-1).values
    for i, conf in enumerate(confidences):
        if conf < threshold:
            return i  # send only tokens 0..i-1
    return len(confidences)  # send all
```

**Impact**: Reduces network payload by 20--30% by cutting low-confidence tails. No training required, no external dependencies.

### Mechanism 3: Hybrid Fallback Mode
...
|------|--------|------------|
| **Added Complexity** | More code paths | Extensive logging of all K decisions; fixed-K fallback always available |
| **Non-Determinism** | Dynamic K makes runs differ | Log K trajectory for post-hoc analysis |
| **Adaptation Lag** | 3--5 requests to converge | Use aggressive EMA ($\alpha=0.5$) for faster response |
| **Cold Start** | No history at start | Default to standard fixed-K for first 10 requests |
...
| High concurrency (>8) | **AHSD wins by 15--25%** | Fewer wasted round-trips per request |
...
```json
{
  ...
  "tokens_proposed": 6,
  "tokens_accepted": 4,
  "tokens_filtered_by_confidence": 1,
...
| **Python** | 3.12.3 |
| **Ianvs** | 0.1.0 (installed from source, editable) |
| **Sedna** | 0.6.0.1 |
| **PyTorch** | 2.9.1 |
| **Transformers** | 4.57.6 |
| **vLLM** | 0.15.1 |
| **Accelerate** | 1.12.0 |
| **Groq SDK** | 1.0.0 |
| **CPU** | 12th Gen Intel Core i5-1235U (12 CPUs, 10 cores) |
| **RAM** | 19 GB |
...
**Benchmark Initialization and Model Loading:**

![Benchmark initialization showing EdgeModel and CloudModel loading, dataset preparation, and inference start](screenshot_benchmark_init.png)

**Rate Limit Error (429) During Cloud API Inference:**

![Rate limit error showing 429 Too Many Requests from Groq API during benchmark execution](screenshot_rate_limit_error.png)
...
**Cache File** (primary evidence): [View cache.json on Google Drive](https://drive.google.com/file/d/16mus7k2T5d2hB4azM-5gegSsi9Z2EIUv/view?usp=sharing)
```bash
...
**Ranking CSV**: [View Ranking Results on Google Drive](https://drive.google.com/file/d/1DokMJehEl7I0M8C5hHUTJa1qTXriMJbh/view?usp=drive_link)
...
**GitHub Issue Document**: [View Issue Document on Google Docs](https://docs.google.com/document/d/1wFC5DaBdCCYxWPcZ74URJsUA-TfW-0OWdKBdQBQsgz0/edit?tab=t.0)
...
### Future Improvement (Optional): Rate limit fallback

> **Note**: This is a separate enhancement, not part of the core fix above. Filed here for completeness; could be a follow-up PR.

Add graceful degradation when Groq API returns 429:

```python
# cloud_model.py: Surface rate limit errors
class RateLimitError(Exception):
    pass

# hard_sample_mining.py: Catch and fallback to edge-only
except RateLimitError:
    self.rate_limit_exceeded = True
    return False  # route remaining to edge
```
...
| Deliverable | Link |
|-------------|------|
| **Resume** | [View Resume (Google Drive)](https://drive.google.com/file/d/1juIiaQFPR6ZVsdHrVlNag5lOYuVPk59G/view?usp=sharing) |
| **Cache File** (15,104 lines, 2.5 MB) | [View cache.json on Google Drive](https://drive.google.com/file/d/16mus7k2T5d2hB4azM-5gegSsi9Z2EIUv/view?usp=sharing) |
| **Ranking Results CSV** | [View Ranking Results on Google Drive](https://drive.google.com/file/d/1DokMJehEl7I0M8C5hHUTJa1qTXriMJbh/view?usp=drive_link) |
| **GitHub Issue Document** | [View Issue Document on Google Docs](https://docs.google.com/document/d/1wFC5DaBdCCYxWPcZ74URJsUA-TfW-0OWdKBdQBQsgz0/edit?tab=t.0) |
| **GitHub Profile** | [github.com/Prachi194agrawal](https://github.com/Prachi194agrawal) |
